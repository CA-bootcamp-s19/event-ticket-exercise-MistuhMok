...
contract EventTickets {
    ...
    */
    function buyTickets(uint numTickets) public payable {
        uint cost = numTickets * TICKET_PRICE;
        require(myEvent.isOpen && msg.value >= cost && numTickets <= myEvent.totalTickets - myEvent.sales);
        
        myEvent.buyers[msg.sender] = numTickets;
        myEvent.sales += numTickets;
        emit LogBuyTickets(msg.sender, numTickets);
        
        if(msg.value > cost) msg.sender.transfer(msg.value - cost);
    }
